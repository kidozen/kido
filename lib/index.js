 ...
 */
 ...
 function Kido ( opts ) {
    ...
    //properties
    this.appStoreScope = 'http://' + opts.hosting + '/';
    ...
    opts.hosting.indexOf('.kidozen.com')   === -1;
 ...
 */
 ...
 Kido.prototype.getAuthConfig = function ( appName, cb ) {
    ...
    var configUrl = self.hosting + "/publicapi/auth/config";
...
Kido.prototype.getAppToken = function (appName, cb ) {

    var self = this;

    if (self._appToken) return cb(null, self._appToken);

    self.login(function ( err, assertion ) {

        if ( err ) return cb(err);

        self.getAuthConfig(appName, function ( err, config ) {
            ...
            var postRequest = {
                ...
                form : {
                    wrap_assertion : assertion,
                    wrap_scope : config.appScope || 'http://management.kidozen.com',
                    wrap_assertion_format : "SAML"
                },
                strictSSL: self.strictSSL
            };
            ...
            request(postRequest, function ( err, res, body ) {
                ...
                self._appToken = 'WRAP access_token="' + token.rawToken + '"';
                cb(null, self._appToken);
            });
        });
});
};
 ...
 */
 ...
 Kido.prototype.deleteApp = function ( id, cb ) {
    ...
    self.DELETE('/api/apps/' + id, function ( err, status ) {
 ...
 */
 ...
 Kido.prototype.deployApp = function ( name, folder, cb ) {
    ...
    self.getAppToken(name, function ( err, token ) {
        var postRequest = {
