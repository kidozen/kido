 ...
 */
 ...
 Kido.prototype.iOSSymbols = function (iosapp, appname, cb ) {
    var self = this;
    getSymbolsFileAndGuid(iosapp,function(err,result) {
        if (err) cb(err);
        zipAppFile(result, function(err, result) {
            if (err) cb(err);
            result.kidoapp = appname;
            result.appurl = self.appTemplate.replace('{name}', appname) 
            uploadZipFile(result,cb);
        });
    });
};
...
function getSymbolsFileAndGuid(filename, cb) {
    var find_cmd = sprintf("find ~/Library/Developer/Xcode -name %1$s.app", filename);
    ...
    child_process.exec(find_cmd, function(err, result) {
        ...
        if (err) cb(err);
        if (result==='') cb(Error.create('file not found'));  
...
function uploadZipFile(options, cb) {
    var url =  options.appurl + '/api/v3/logging/crash/ios/symbols/' + options.zipFile;
    var post = fs.createReadStream( getUserHome() + '/' + options.zipFile).pipe (request.post(url));
    post.on('error', function(err) {
        cb(err);
    });
    post.on('end', function(err) {
        cb(null, options);
    });    
}
